...
public class trSQLiteDataHelper <T> implements trIDAO<T> {
     ...
     */
    public trSQLiteDataHelper(Class<T> dtoClass, String tableName, String tablePK) throws AppException {
        try {
            openConnection();
        } catch (SQLException e) {
            throw new AppException(null, e, getClass(), "trSQLiteDataHelper");
        }
    ...
    public boolean create(T entity) throws AppException {
        ...
        for (Field field : fields) {
            ...
            if (!name.equalsIgnoreCase(tablePK)
                && !name.equalsIgnoreCase("Estado") && !name.equalsIgnoreCase("trEstado")
                && !name.equalsIgnoreCase("FechaCreacion") && !name.equalsIgnoreCase("trFechaCreacion")
                && !name.equalsIgnoreCase("FechaModifica") && !name.equalsIgnoreCase("trFechaModifica")) {
    ...
    public boolean update(T entity) throws AppException {
        try {
            ...
            updates.append("trFechaModifica = ?"); // campo técnico de auditoría (o FechaModifica si la tabla lo usa)
            ...
            try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
                ...
                for (Field field : fields) {
                    String name = field.getName();
                    if (!name.equalsIgnoreCase(tablePK)
                        && !name.equalsIgnoreCase("trEstado") && !name.equalsIgnoreCase("trEstado")
                        && !name.equalsIgnoreCase("FechaCreacion") && !name.equalsIgnoreCase("trFechaCreacion")
                        && !name.equalsIgnoreCase("FechaModifica") && !name.equalsIgnoreCase("trFechaModifica")) {
                        if (!field.canAccess(entity)) {
                            field.setAccessible(true);
                ...
                stmt.setString(index++, getDataTimeNow()); // trFechaModifica
    ...
    public Integer getMaxReg(String tableCelName) throws AppException {
        String sql = String.format("SELECT MAX(%s) FROM %s WHERE trEstado = 'A'", tableCelName, tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
    ...
    public Integer getMinReg(String tableCelName) throws AppException {
        String sql = String.format("SELECT MIN(%s) FROM %s WHERE trEstado = 'A'", tableCelName, tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
